...
## Brand pin (AI Her Way)

This fork of the skill is pinned to AI Her Way's locked Social/Reels brand system. The generic methodology below is unchanged; these pins only fill the choices the framework deliberately leaves open (which fonts, which single accent hue, which colour for the rule device). Where the generic text describes a role or option, apply the brand value:

- Typography — no exceptions: `hero` = Playfair Display Black/900; `editorial accent` = Playfair Display Italic; `support` = Montserrat (regular/medium).
- Single-hue accent rule: exactly one italic word per group gets a colour accent — Walnut `#7B5A49` on light footage, Dusty Blue `#C4D6DA` on dark footage. This is the brand's single most recognisable element; never apply it to more than one word per group, and never use an arbitrary hue.
- Gold hairline: the `rule-draw` device renders in Gold `#B08A62`, matching the brand's thin (~2px) gold rule under headlines.

Full brand palette for reference (use a concrete hex only where genuinely needed): Espresso `#35241F`, Ink `#2B1E19`, Linen Cream `#F2EBDD`, Ivory `#FAF6EF`, Dusty Blue `#C4D6DA`, Stone `#C0CACE`, Sage `#AAB6A0`, Deep Olive `#46513D`, Walnut `#7B5A49`, Gold `#B08A62`.
...
- support: a clean, locally bundled sans in its real regular or medium weight (brand pin: Montserrat);
- hero: a genuine extra-bold, black, or condensed display face whose font file contains that exact weight (brand pin: Playfair Display Black/900);
- editorial accent: an occasional serif or italic face for a conversational action, aside, or CTA—not for every cue (brand pin: Playfair Display Italic).
...
Use silver-white for a clean glass sequence. Introduce one hue per word only when the brand, footage, or meaning benefits from it; do not turn one word into a pastel rainbow. For this project, that one hue is always Walnut `#7B5A49` on light footage or Dusty Blue `#C4D6DA` on dark footage, applied to the single `editorial accent` word per the brand's italic-accent rule—never to more than one word per group. Reserve multicolor, flag, map, image, or video texture for speech that semantically justifies it.
...
- `rule-draw`: short underline or divider that draws after the anchor arrives (brand pin: render in Gold `#B08A62`, matching the brand's gold-hairline signature device);
...
```json
{
  ...
  "accent": "#7B5A49",
  "_accentNote": "Walnut for light grounds; use Dusty Blue #C4D6DA on dark grounds",
  "cues": [
    {
      "id": "cc-001",
